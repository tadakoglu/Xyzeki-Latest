...
import { BlobRepository } from 'src/app/model/repository/blob-repository';
import { BlobsService } from 'src/app/model/services/blobs.service';
...
export class BlobsComponent implements OnInit, AfterViewInit, OnDestroy {
  constructor(public repository: BlobRepository, public _location: Location, private repositoryTM: TeamMemberRepository, private dataService: DataService, private containerExistingRepo: ContainerRepository, 
    
    private blobService: BlobsService, public mLicenseRepository: MemberLicenseRepository,
  ...
  ngOnInit(): void {
    ...
    //   //this.repository = new FileRepository(this.containerName, blobService, signalService);
    //   this.repository.loadAll(this.containerName);
    // })

    this.route.data.subscribe((resolvedData: { containerBlobs: CloudFiles }) => {
      this.containerName = this.route.snapshot.paramMap.get('ContainerName')
      this.repository.loadContainerFilesViaResolver(resolvedData.containerBlobs, this.containerName);
    })
